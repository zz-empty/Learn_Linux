...
int main()
{
    ...
    printf("[info] udp_server init succeed! ready chat...\n");
    while (1) {
        ...
        for (int i = 0; i < readyNum; ++i) {
            if (evs[i].data.fd == server_fd) {
                ...
                buffer.len = recvfrom(server_fd, buffer.data, MAX_DATA_LEN, 0, (struct sockaddr*)&cliAddr, &clilen);
                RET_CHECK(buffer.len, -1, "recvfrom");

                // 显示数据
                printf("[client_ip %s] %s\n", inet_ntoa(cliAddr.sin_addr), buffer.data);
            }
            ...
            if (evs[i].data.fd == STDIN_FILENO) {
                ...
                ret = sendto(server_fd, buffer.data, buffer.len - 1, 0, (struct sockaddr*)&cliAddr, clilen);
