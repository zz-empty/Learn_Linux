...
void sigfunc(int signum) {
    ...
    const char *msg = "[parent] signal received!\n";
...
int main(int argc, char **argv) {
    ...
    while (1) {
        ...
        if (-1 == readyNum) {
            // 信号处理函数会中断低速系统调用，不会自动重启
            if (errno == EINTR) {
                printf("[info] epoll_wait interrupted by signal!\n");
                continue;  // 被信号中断，忽略中断，重新等待
            }
            perror("epoll_wait");   // 其他错误才退出
            exit(EXIT_FAILURE);
        }
