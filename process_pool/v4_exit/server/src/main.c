...
int main(int argc, char **argv)
{
    ...
    ret = epoll_add(epfd, server_fd);   // tcp监听套接字
    RET_CHECK(ret, -1, "epoll_add");
    ret = epoll_add(epfd, exit_pipes[0]);   // 退出管道
    RET_CHECK(ret, -1, "epoll_add");
    ...
    while (1) {
        ...
        for (int i = 0; i < readyNum; ++i) {
            if (evs[i].data.fd == exit_pipes[0]) {
                ...
                for (int j = 0; j < pool.size; ++j) {
                    int status;
                    waitpid(pool.workers[i].pid, &status, 0);
                    if (WIFEXITED(status)) {
                        printf("[info] exit num: %d\n", WEXITSTATUS(status));
                    }
                }
                // 退出主进程
                printf("[signal] main exit!\n");
                close(server_fd);
                free(pool.workers);
                exit(0);
            }
            if (evs[i].data.fd == server_fd) {
                ...
                for (int j = 0; j < pool.size; ++j) {
                    if (pool.workers[j].busy == 0) {
                        send_fd(pool.workers[j].pipefd, client_fd, 0);
